...
- **遗漏的重要 caveat**：学界对这个话题有重要的 counterexample、nuance、正在争论的问题，但文章没提——**仅在不提会让读者做出错误决定时**才算 Warning；纯"可以顺便提"的 nuance 是 Minor
- **过度简化**：把 X 说成 Y，**且 Y 会把读者引向错误结论**——是 Critical。如果 Y 只是 X 的合理简化、读者按 Y 理解也能做对事，那**不是** Critical 也不是 Warning
...
- **涉及目标代码库的概念/行为**：如果 plan.md 的"代码库位置"或"权威源"节列了**本地路径**，**必须**用 Grep / Glob / Read 本地验证；**禁止**对同一个仓库用 WebFetch 抓 GitHub 页面或 WebSearch 找二手博客。本地代码是该仓库概念的权威一手源。如果 plan.md 记了 commit/tag，尽量在该版本验证；当前 checkout 与记录不一致时在报告里标明。
...
- **"读者心智模型被引到错误方向"才是 Critical 的判定线**（**最重要的硬规则**）：
  - 读者按文章建立的心智模型，在后续真实应用时会**得出错误结论** → Critical
  - 读者建立的是**简化但方向正确**的模型，真实应用时顶多多查一下细节 → **不是 Critical 也不是 Warning**，至多 Minor
  - **"可以更精确"、"更严谨"、"还有 edge case 没讲"这类不是 Warning，最多是 Minor**。科普文本来就是简化——在读者不会被引到错误方向的前提下，简化是正确的、不是问题
  - 自检：在把一条标为 Critical/Warning 之前，问自己"如果读者按文章现在这样理解了这个概念，他在 [典型使用场景] 下会做错什么决定吗？"；想不出具体错误决定的 → 降级为 Minor 或不报
- **"科普允许简化，但不允许错误"**——上面那条硬规则的概括：读者按这个解释建立的心智模型，如果在需要的时候会把他引到正确的地方，就 OK。
- **认权威源**（**硬规则**）：
