...
- 合集归属在 brainstorm 阶段决定，会记录到 `.composir/<slug>-brainstorm.md` 和 `.composir/<slug>-plan.md` 里
- 如果文章不属于任何合集，省略这句话
...
### 3. 标点必须匹配文章语言
Claude Code 默认输出 ASCII 半角标点（`,` `.` `:` `;` `!` `?` `(` `)` `...` 等）。**中文文章必须主动切换到全角标点**，否则正文会出现"半角逗号 + 中文字"这种刺眼组合。
...
**例外（无论文章语言都保持原样）**：
- 代码块和行内代码内部的标点
- URL、文件路径、命令、变量名、API 名等技术 token
- 数字、版本号、日期里的点和冒号（如 `5.0`、`2026-04-19`、`12:30`）
- 中文段落里嵌入的英文术语 / 专有名词的原文（如 `Claude Code`、`fact-checker`）
